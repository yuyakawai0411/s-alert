class UsersController < ApplicationController
  ...
  before_action :side_menu, only: [:show, :edit, :post_cards, :favorite_cards]
  ...
  def post_cards
    @cards = @user.cards.page(params[:page]).per(3)
  end 
  
  def favorite_cards
    @favorites = @user.fav_cards.page(params[:page]).per(3)
  end
