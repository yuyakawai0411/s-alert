...
- [:muscle: 苦労した点](#muscle-苦労した点)
- [:clap: 工夫した点](#clap-工夫した点)
...
# :muscle: 苦労した点
オリジナルアプリを本番環境にデプロイした際に、双方向通信が正常に動作しませんでした。

そこで、本番環境でもヘルスチェックやシェルを用いて、実行Logを確認するようにしました。また、コードの変更はこまめにGitHubにcommitすることでエラーの原因を絞り込みやすくし、無事エラーを解消することができました。

今後とも、適切なcommitを心がけ、エラーが起きた際には、実行Logやコードの変更履歴をもとに原因を切り分けながら、開発していきます。

[↑目次](#top)

# :clap: 工夫した点
オリジナルアプリの追加実装を行なった際に、予期せぬエラーが発生し、CircleCIのテストが落ちることがありました。

そこで、クラスの抽出等を行い、個々の責務を明確にするようにしました。加えて、設定ファイルを開発用と本番用で分離、ビューの変更に強いブラウザテスト等を意識することで、コードの変更をスムーズに本番環境に反映できるようにしました。

今後も、リファクタリングを怠らず、チーム内でのルールを確認しながら、保守性の高いコードを目指します。
